...
func BenchmarkServeWrite(b *testing.B) {
	...
	data, err := ioutil.ReadFile("./testdata/thousandMetric")
	if err != nil {
		b.Error(err)
	}

	listener.Start(&testutil.Accumulator{})
	url := createURL(listener, "http", "/write", "db=mydb")
...
func BenchmarkServeWriteConcurrent(b *testing.B) {
	data, err := ioutil.ReadFile("./testdata/thousandMetric")
	if err != nil {
		b.Error(err)
	}
		...
		for n := 0; n < b.N; n++ {
			for i := range reqs {
				...
				go func(i int) {
					...
					listener.serveWrite(resps[i], reqs[i])

					if http.StatusNoContent != resps[i].Code {
						b.Errorf("Status codes not equal %d:%d", http.StatusNoContent, resps[i].Code)
					}
				}(i)
			}
			wg.Wait()
